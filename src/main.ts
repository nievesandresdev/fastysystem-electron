...
async function waitForFrontend(maxRetries = 80, delay = 1000): Promise<boolean> {
...
function createWindow() {
  ...
  if (app.isPackaged) {
    ...
    iconPath = path.join(appPath, '..', 'frontend', 'public', 'img', 'fasty_logo.ico');
    
    // Si no se encuentra ahí, intentar con process.resourcesPath
    if (!fs.existsSync(iconPath)) {
      iconPath = path.join(process.resourcesPath, '..', 'frontend', 'public', 'img', 'fasty_logo.ico');
    }
  } else {
    // En desarrollo, buscar en la estructura del proyecto
    iconPath = path.join(__dirname, '..', '..', 'frontend', 'public', 'img', 'fasty_logo.ico');
  }
